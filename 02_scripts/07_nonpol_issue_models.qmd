---
title: "Analysis of Motivated Reasoning on Nonpolitical Topics Using Issue Motives"
format:
  html:
    theme:
      light: lux
...
model_dir <- here("03_output", "models", "nonpol_issue")
dia_dir <- here("03_output", "diagnostics", "nonpol_issue")
...
m1.per: response_proportion \~ issue_motive \* question_topic + (issue_motive \| subj_idx)
...
m1.neu: response_proportion \~ issue_motive \* question_topic + (issue_motive \| subj_idx)
...
m2.per: response_proportion \~ issue_motive_strength \* question_topic + (issue_motive_strength \| subj_idx)

Set model formula

```{r formula-m2.per}
f_m2.per <- bf(response_proportion ~ 
                    mo(issue_motive_strength) * question_topic +
                    (mo(issue_motive_strength) | subj_idx), center = T)
```
...
m2.neu: response_proportion \~ issue_motive_strength \* question_topic + (issue_motive_strength \| subj_idx)
...
m3.per: response_proportion \~ issue_motive \* crt \* question_topic + (issue_motive \| subj_idx)

Set model formula

```{r formula-m3.per}
f_m3.per <- bf(response_proportion ~ issue_motive * scale(crt_correct) *
                    question_topic + 
                    (issue_motive | subj_idx), center = T)
```
...
m3.neu: response_proportion \~ issue_motive \* crt \* question_topic + (issue_motive \| subj_idx)

Set model formula

```{r formula-m3.neu}
f_m3.neu <- bf(response_proportion ~ issue_motive * scale(crt_correct) *
                    question_topic + 
                    (issue_motive | subj_idx), center = T)
```
...
m4.per: response_proportion \~ issue_motive \* commission_errors_r \* question_topic + (issue_motive \| subj_idx)
...
m4.neu: response_proportion \~ issue_motive \* commission_errors_r \* question_topic + (issue_motive \| subj_idx)
...
m5.per: response_proportion \~ issue_motive_strength \* crt \* question_topic + (issue_motive_strength \| subj_idx)

Set model formula

```{r formula-m5.per}
f_m5.per <- bf(response_proportion ~ mo(issue_motive_strength) * scale(crt_correct) * question_topic + 
                 (mo(issue_motive_strength) | subj_idx), center = T)
```
...
m5.neu: response_proportion \~ issue_motive_strength \* crt \* question_topic + (issue_motive_strength \| subj_idx)

Set model formula

```{r formula-m5.neu}
f_m5.neu <- bf(response_proportion ~ mo(issue_motive_strength) * scale(crt_correct) * question_topic + 
                 (mo(issue_motive_strength) | subj_idx), center = T)
```
...
m6.per: response_proportion \~ issue_motive_strength \* commission_errors_r \* question_topic + (issue_motive_strength \| subj_idx)

Set model formula

```{r formula-m6.per}
f_m6.per <- bf(response_proportion ~ mo(issue_motive_strength) * scale(commission_errors_r) * question_topic + 
                 (mo(issue_motive_strength) | subj_idx), center = T)
```
...
m6.neu: response_proportion \~ issue_motive_strength \* commission_errors_r \* question_topic + (issue_motive_strength \| subj_idx)

Set model formula

```{r formula-m6.neu}
f_m6.neu <- bf(response_proportion ~ mo(issue_motive_strength) * scale(commission_errors_r) * question_topic + 
                 (mo(issue_motive_strength) | subj_idx), center = T)
```
